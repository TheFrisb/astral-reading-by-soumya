    ...
    def handle(self, *args, **options):
        ...
        for tag in data:
            try:
                ...
                response = requests.get(tag["image"])
                response.raise_for_status()  # Raise an exception for bad status codes
                # Create a ContentFile with the image content and set a filename
                image_content = ContentFile(response.content)
                image_content.name = f"{tag['name'].lower()}.png"
                # Create Category instance with name and image
                obj = Category(name=tag["name"], image=image_content)
                obj.save()  # Save the instance, which also saves the image
                categories.append(obj)
            except requests.RequestException as e:
                self.stderr.write(
                    self.style.ERROR(f"Failed to download image for {tag['name']}: {e}")
                )
                obj = Category(name=tag["name"])
                obj.save()
                categories.append(obj)
        ...
        for i in range(50):
            ...
            obj = BlogPost.objects.create(
                title=f"Blog post {i + 1} about {category.name}",
                content=f"This is blog post {i + 1} about {category.name}",
            )
